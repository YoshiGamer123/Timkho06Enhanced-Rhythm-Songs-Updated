...
init_doors = function()
{
    ...
    for (var i = 0; i < 1; i++)
    {
        scr_rhythmgame_song_select(song_ids[2]);
        var _file = local_preview;
        var _track_id = song_ids[selected_song_id];
        var door = create_door(2, song_ids[2]);
        var rank_data = [highscore_rank[_track_id], highscore_rank_hard[_track_id]];
        var symbol = create_symbol(door, song_ids[2], rank_data);
...
update_scroller = function()
{
    ...
    song_data[5] = _song_id;
    scroller.init_song(song_data);
    scroller.update_banner();
    var rank_data = [highscore_rank[selected_song_id], highscore_rank_hard[selected_song_id]];
    symbol_dont_forget.init_ranks(rank_data);
};
