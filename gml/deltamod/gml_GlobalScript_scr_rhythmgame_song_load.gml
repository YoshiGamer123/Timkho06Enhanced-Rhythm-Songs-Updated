function scr_rhythmgame_load_song(arg0 = 0, arg1 = true, arg2 = true, arg3 = false)
{
        ...
        else if (arg0 >= 11)
    {
        ...
        var _file = file_text_open_read(program_directory + "/SongCharts/music_timing_customsong_info" + string(custom_song_num) + ".txt");
        var l_arg = [];
        var line_ind = 0;
        ...
        try
        {
            l_arg[2] = real(l_arg[2]);
        }
        catch (e)
        {
        }
        
        if (!is_string(l_arg[2]))
            track_length = l_arg[2];
        else
            track_length = audio_sound_length(snd_init(track1_id));
        
        bpm = real(l_arg[3]);
        notespeed = real(l_arg[4]);
        
        try
        {
            l_arg[8] = real(l_arg[8]);
        }
        catch (e)
        {
        }
        
        if (!is_string(l_arg[8]))
            trackstart = l_arg[8];
        else
            trackstart = scr_round_to_beat(-2 * (240 / bpm), bpm);
...
function scr_rhythmgame_song_select(arg0)
{
    ...
    if (arg0 >= 5 && file_exists(program_directory + "/SongCharts/music_timing_customsong_info" + string(arg0 - 4) + ".txt"))
    {
        song_id = 6 + arg0;
        vinyl_id = 1;
        var _file = file_text_open_read(program_directory + "/SongCharts/music_timing_customsong_info" + string(arg0 - 4) + ".txt");
        var l_arg = [];
        var line_ind = 0;
        ...
        preview = l_arg[6];
        alt_preview = l_arg[7];
        local_preview = snd_init(alt_preview);
        chapter = 3;
        bpm = real(l_arg[3]);
        _file = -1;
    }
...
function scr_rhythmgame_song_select_id(arg0)
{
    if (arg0 == 0)
        ...
        return 2;
    else if (arg0 >= 5 && file_exists(program_directory + "/SongCharts/music_timing_customsong_info" + string(arg0 - 4) + ".txt"))
        return 6 + arg0;
}
...
function scr_rhythmgame_rank_scores(arg0, arg1 = 0)
{
    ...
    if (arg0 >= 11 && file_exists(program_directory + "/SongCharts/music_timing_customsong_info" + string(arg0 - 10) + ".txt"))
    {
        var _file = file_text_open_read(program_directory + "/SongCharts/music_timing_customsong_info" + string(arg0 - 10) + ".txt");
        var file_rank = [];
        ...
        if (_file != -1)
        {
            ...
            {
                var line = file_text_read_string(_file);
                var l_arg = string_split(line, ",");
                
                if (array_length(l_arg) >= 5)
                    file_rank = [real(l_arg[0]), real(l_arg[1]), real(l_arg[2]), real(l_arg[3]), real(l_arg[4])];
...
function scr_rhythmgame_load_events(arg0)
{
    ...
    else if (arg0 >= 11 && file_exists(program_directory + "/SongCharts/music_timing_customsong_events" + string(arg0 - 10) + ".txt"))
    {
        drums.auto_play = 1;
        var _file = file_text_open_read(program_directory + "/SongCharts/music_timing_customsong_events" + string(arg0 - 10) + ".txt");
        ...
        if (_file != -1)
        {
            while (!file_text_eof(_file))
            {
                ...
                if (array_length(l_arg) >= 5 && l_arg[4] == "sprite_get_name")
                {
                    if (l_arg[0] == "vocals")
                    {
                        with (vocals.performer)
                            scr_music_event_add(real(l_arg[1]), l_arg[2], asset_get_index(l_arg[3]), true);
                    }
                ...
                else
                {
                    ...
                    {
                        with (vocals.performer)
                            scr_music_event_add(real(l_arg[1]), l_arg[2], real(l_arg[3]), true);
                    }
