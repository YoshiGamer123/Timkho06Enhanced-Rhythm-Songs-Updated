...
function scr_rhythmgame_draw_backing(arg0, arg1, arg2 = false, arg3 = 0)
{
    ...
    if (room != room_dw_rhythm)
        ossafe_fill_rectangle(arg1 - 40, bottomy - 200, arg1 + 40, bottomy + 200, false);
    else
...
function scr_rhythmgame_draw_border(arg0, arg1 = false)
{
    ...
    if (!arg1 && !demo_mode)
    {
        ...
        if (instrument == 0)
        {
            _name = _jp ? asset_get_index("spr_ja_bnamekris") : asset_get_index("spr_bnamekris");
            _hoff = 21;
        }
        else if (instrument == 1)
        {
            _name = _jp ? asset_get_index("spr_ja_bnamesusie") : asset_get_index("spr_bnamesusie");
            _hoff = 24;
        }
        else if (instrument == 2)
        {
            _name = _jp ? asset_get_index("spr_ja_bnameralsei") : asset_get_index("spr_bnameralsei");
            _hoff = 30;
        }
...
function scr_rhythmgame_songlist()
{
    ...
    else
    {
        ...
        if (instance_exists(obj_fusionmenu) && obj_fusionmenu.dojoFlag[0] == -1)
        {
            dojoEncounterAmbush[0] = sprite_add(program_directory + "/SongAlbums/customalbum1.png", 0, false, false, false, true, 0, 0);
            dojoEncounterAmbush[1] = sprite_add(program_directory + "/SongAlbums/customalbum2.png", 0, false, false, false, true, 0, 0);
            dojoEncounterAmbush[2] = sprite_add(program_directory + "/SongAlbums/customalbum3.png", 0, false, false, false, true, 0, 0);
            dojoEncounterAmbush[3] = sprite_add(program_directory + "/SongAlbums/customalbum4.png", 0, false, false, false, true, 0, 0);
            dojoEncounterAmbush[4] = sprite_add(program_directory + "/SongAlbums/customalbum5.png", 0, false, false, false, true, 0, 0);
            dojoFlag[0] = 0;
            dojoFlag[1] = 1;
            dojoFlag[2] = 2;
            dojoFlag[3] = 3;
            dojoFlag[4] = 4;
            dojoName[0] = "battle_preview_crisp.ogg";
            dojoName[1] = "field_of_hopes_preview_crisp.ogg";
            dojoName[2] = 513;
            dojoName[3] = 86;
            dojoName[4] = 161;
        }
        
        for (var i = 5; i < (CustomID + 20); i++)
        {
            if (file_exists(program_directory + "/SongCharts/music_timing_customsong_info" + string(i - 4) + ".txt"))
            {
                var _file = file_text_open_read(program_directory + "/SongCharts/music_timing_customsong_info" + string(i - 4) + ".txt");
                var l_arg = [];
                var line_ind = 0;
                ...
                _trackname[CustomID] = l_arg[9];
                dojoFlag[CustomID] = i;
                var local_audio = l_arg[7];
                dojoName[CustomID] = local_audio;
                
                try
                {
                    if (array_length(dojoEncounterAmbush) < (CustomID + 1))
                        dojoEncounterAmbush[CustomID] = sprite_add(program_directory + "/SongAlbums/" + l_arg[11] + ".png", 0, false, false, false, true, 0, 0);
                }
                catch (_exception)
                {
                    dojoEncounterAmbush[CustomID] = undefined;
                }
